...
async function avisar(app, evento, { para, de, lang, vars = {} } = {}) {
  ...
  // ── REGRA 3: QUEM DESLIGOU O ASSUNTO NÃO RECEBE ──────────────────────
  //
  // A escolha é por ASSUNTO e vale para os dois canais — o mesmo "treino novo"
  // que sai daqui em push sai por e-mail noutro lugar. Ver
  // `lib/notificacoes.js`.
  //
  // A leitura falha? Manda. Perder um aviso porque o banco tossiu é pior que
  // um aviso a mais, e o padrão desta lista é receber.
  try {
    const dono = await app.api.user.data(para);
    if (!notificacoes.querReceber(dono, evento)) return { ok: false, erro: "desligado_pela_pessoa" };
  } catch (erro) {
    console.warn("[avisar] não consegui ler a preferência:", erro.message);
  }
