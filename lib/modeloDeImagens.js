...
function modeloDeImagens({ collection, dono, prefixo }) {
  ...
  Modelo.prototype.save = async function (donoId, mime, buffer) {
    ...
    // ── SE A GRAVAÇÃO FALHAR, A FICHA VAI JUNTO ──────────────────────────
    //
    // O documento entra antes porque a chave do arquivo precisa do id. Mas o
    // que vem depois pode estourar — e estourou: a pasta não estava declarada
    // em `lib/arquivos.js`, e o que ficou no banco foi uma ficha de foto com
    // `mime` e `size` e nenhum byte em lugar nenhum.
    //
    // Uma imagem que não é imagem é pior que imagem nenhuma: ela conta como
    // foto na faxina, aparece na contagem, e a rota que a lê devolve 404 para
    // um id que existe. O erro continua subindo — quem chamou precisa saber —,
    // mas sem deixar o rastro.
    try {
      ...
      await col.deleteOne({ _id: r.insertedId }).catch(() => {});
      throw erro;
    }
