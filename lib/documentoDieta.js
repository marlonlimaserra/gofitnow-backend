...
function refeicao(t, m, lang) {
  ...
  const linhas = alimentos
    .map((a) => {
      ...
      return `<tr>
        <td style="padding:4px 0;border-top:1px solid ${LINHA};font-size:12.5px;${estilo}">${prefixo}${escapar(a.name)}</td>
...
function documentoDieta({ diet, person, lang = "pt-BR", fuso = "UTC", marca = null }) {
  ...
  const refeicoes = (diet.meals || []).map((m) => refeicao(t, m, lang)).join("");
