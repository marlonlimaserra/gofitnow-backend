...
const {
  TINTA, FRACO, LINHA, escapar, formatarData, secao, cartoes, pagina,
...
// O dinheiro na língua da MOEDA, e não na de quem lê.
//
// R$ 1.234,56 é assim em qualquer idioma: o separador pertence à moeda, não ao
// leitor. É a mesma escolha do `formatarMoeda` da tela (`lib/dinheiro.js`) — as
// duas precisam escrever o mesmo número, senão a folha discorda do que estava na
// tela de quem a imprimiu.
function dinheiro(centavos, moeda) {
  const valor = Number(centavos);
  if (!Number.isFinite(valor)) return "";

  const code = currencies.normalize(moeda);
  try {
    return new Intl.NumberFormat(currencies.find(code)?.locale || "pt-BR", {
      style: "currency",
      currency: code,
    }).format(valor / 100);
  } catch (erro) {
    // Moeda que o `Intl` desta máquina não conhece não pode derrubar o extrato:
    // sai o número e o código ao lado, que é feio e legível.
    return `${code} ${(valor / 100).toFixed(2)}`;
  }
}
...
function ficha(t, person, foto, lang, fuso) {
  ...
  return `<table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="margin-top:18px;">
    <tr>
      ...
      <td valign="middle">
        <div style="font-size:14px;font-weight:700;color:${TINTA};">${escapar(person?.name || "")}</div>
        ${contato}
      </td>
    </tr>
  </table>`;
...
const CABECA = `font-size:10px;font-weight:700;letter-spacing:.05em;text-transform:uppercase;color:${FRACO};padding:0 0 6px;border-bottom:1px solid ${LINHA};`;
const CELULA = `font-size:12px;color:${TINTA};padding:8px 0;border-bottom:1px solid #f1f5f9;vertical-align:top;`;
...
function tabelaDeCobrancas(t, cobrancas, pagamentosPor, moedaPadrao, lang, fuso, nomeDaForma) {
  ...
  const linhas = cobrancas
    .map((c) => {
      ...
      const cobranca = `<tr>
        ...
        <td style="${CELULA}text-align:right;white-space:nowrap;">${escapar(dinheiro(falta, moeda))}</td>
        <td style="${CELULA}white-space:nowrap;color:${sit.cor};font-weight:600;">${escapar(sit.texto)}</td>
      </tr>`;
      ...
      const recebidos = doLote
        .map(
          (p) => `<tr>
            <td colspan="2" style="${CELULA}padding-left:14px;color:${FRACO};">
  ...
  return `<table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="border-collapse:collapse;">
    <tr>
      ...
      <th align="left" style="${CABECA}">${escapar(t("finance.colStatus"))}</th>
    </tr>
    ${linhas}
  </table>`;
}
...
function tabelaDeAvulsos(t, pagamentos, moedaPadrao, lang, fuso, nomeDaForma) {
  ...
  const linhas = pagamentos
    .map(
      (p) => `<tr>
        <td style="${CELULA}white-space:nowrap;">${escapar(formatarData(p.date, lang, fuso))}</td>
        <td style="${CELULA}">${escapar(nomeDaForma(p.method))}${p.note ? " · " + escapar(p.note) : ""}</td>
        <td style="${CELULA}text-align:right;white-space:nowrap;color:${entrou(p) ? "#059669" : FRACO};font-weight:600;">${escapar(dinheiro(p.amount, p.currency || moedaPadrao))}</td>
      </tr>`
    )
    .join("");
  ...
  return `<table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="border-collapse:collapse;">
    <tr>
      <th align="left" style="${CABECA}">${escapar(t("finance.date"))}</th>
...
function documentoFinanceiro({
  ...
  const corpo = [
    ...
    secao(
      t("finance.charges"),
      tabelaDeCobrancas(t, charges, pagamentosPor, moeda, lang, fuso, nomeDaForma)
    ),
  ...
  return pagina({
    ...
    nome: person?.name,
    subtitulo: `${t("finance.statement")} · ${formatarData(emitidoEm, lang, fuso)}`,
    corpo,
    rodape: `${person?.name || ""} · ${formatarData(emitidoEm, lang, fuso)}`,
    marca,
  });
}
