...
// A volta do Google entra aqui pelo mesmo motivo, e por um caminho pior de
// descobrir: ela chega do servidor do Google, sem cabeçalho nenhum nosso, e com
// Host `backend.gofitnow.fit` — que `lib/instance.js` ignora de propósito. Sem
// esta linha a resposta seria 400 `no_instance` DEPOIS de a pessoa já ter
// consentido na tela do Google, e o sintoma na cara dela seria uma página de
// erro no meio do login. A instância da volta sai do `state`, que é o único
// lugar confiável: foi este servidor que a gravou lá na ida, já conferida.
//
// `/health` entrou em 07/09/2026, preparando a saída do VPS. Ele PINGA o Mongo, e
// não precisa de instância nenhuma para isso — mas respondia 400 de fora, o que
// é a pior resposta possível para uma sonda: quem olha lê "servidor quebrado"
// quando o servidor está inteiro.
//
// A sonda que o balanceador vai usar é `/`, não esta: uma verificação que
// consulta o BANCO derruba toda a frota quando o Atlas pisca — transforma um
// soluço do banco numa queda total, porque o balanceador mata o que está
// "doente". Dependência se olha em monitoramento; vida do processo se olha em `/`.
const SEM_INSTANCIA = [
  /^\/$/,
  /^\/health$/,
  /^\/public\//,
