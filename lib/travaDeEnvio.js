...
// ── COM REDIS, A TRAVA VALE PARA TODAS AS MÁQUINAS ───────────────────────
//
// É a que mais sofre com duas máquinas: a trava de cinco minutos passa a deixar
// sair UM e-mail por servidor. Com quatro, quem apertar "esqueci minha senha"
// quatro vezes recebe quatro mensagens — que é exatamente o que ela existe para
// impedir.
//
// Aqui a marca é uma chave com PRAZO, e não uma janela deslizante: a pergunta é
// "quanto falta para poder de novo", e o próprio Redis já a responde com o
// tempo de vida restante da chave.
function marcarEnvio(chave) {
  if (redis.ligado()) redis.marcarComPrazo(`envio:${chave}`, JANELA_MS);

  if (noWorker()) process.send({ tipo: AVISO, chave });
  else marcar(chave);
}

async function faltamSegundos(chave, janela = JANELA_MS) {
  if (redis.ligado()) {
    const restaMs = await redis.faltaDoPrazo(`envio:${chave}`);
    if (restaMs !== null) {
      // A janela pedida pode ser MENOR que a gravada (a central deixa configurar
      // o tempo). Nesse caso vale a pedida: o que passou dela já não trava.
      const gasto = JANELA_MS - restaMs;
      const falta = janela - gasto;
