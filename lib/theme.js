...
const PADRAO = {
  // ── A COR OFICIAL, e o que um cliente NOVO recebe (28/08/2026) ──────────
  //
  // O azul-céu da instância `marlon`, que virou a cor oficial do produto. A
  // decisão veio dele, apontando o cabeçalho do próprio sistema: *"lá no header
  // tem essa combinação degradê bem legal"* — e o caminho foi o inverso do
  // habitual, com o site e o padrão copiando o que o produto já usava.
  //
  // Antes daqui morava o verde `#16a34a`, medido no wordmark. Ele continua
  // existindo como PRESET (`green`, lá embaixo) para quem quiser: o que mudou é
  // só qual sai de fábrica.
  //
  // Repare que o botão NÃO vai sair exatamente nesta cor, e não é defeito: o
  // gerador de escala joga fora a luminosidade e crava a de cada tom (600 = 43%).
  // O que importa aqui é dar ao gerador o MATIZ e a SATURAÇÃO certos.
  //
  // ── 10/09/2026: o LARANJA da VAFIT ──
  //
  // Primeiro entrou o azul `#1a4085` (o "VA"), e estava errado: o fundo da
  // tela de entrar já é marinho, então botão azul sobre fundo azul sumia e a
  // tela inteira ficava monocromática — sem nada do laranja que é metade da
  // marca.
  //
  // No logo o azul é o corpo e o LARANJA é o acento: é ele que marca o "FIT"
  // e as fitas em movimento. Numa interface, acento é o que vira ação. Daí
  // `#fd7824`, o laranja medido do arquivo.
  //
  // O ciano antigo continua como preset `oceano`, e o azul da VAFIT está
  // disponível para quem quiser inverter.
  //
  // O que a troca NÃO muda: `scale()` usa só matiz e saturação desta cor, e
  // impõe as luzes da tabela `LUZES`. Então o tom 500 sai `#2a67d5`, um azul
  // médio — não este marinho. Escurecer a semente não escurece a escala.
  //
  // O que ela muda de verdade, e é o cuidado: o front tem os dez valores
  // ESTÁTICOS em `src/index.css` para pintar antes de o tema chegar, e eles
  // têm de ser byte a byte o que esta linha gera. Mudar aqui sem mudar lá faz
  // toda abertura piscar de uma escala para a outra.
  //
  // A observação antiga continua valendo: o texto do botão sai de comparação
  // de contraste (`--brand-fg`), então ele se resolve sozinho.
  brand: "#fd6121",
  ...
  // como campo.
  // Sem segunda cor: botão CHAPADO no verde da marca, como sempre foi.
  brandTo: "",
  layout: "centered",
  background: "gradient",
  ...
  // ── O FUNDO PADRÃO É CRAVADO, e não derivado da marca ────────────────────
  //
  // Vazio aqui significa "derive da cor da marca", e era o que fazia sentido
  // quando a marca era azul: azul escurecido dá um marinho bonito. Com a marca
  // LIMA o mesmo cálculo dá um verde-oliva lavado — a cor que sobra de escurecer
  // amarelo-esverdeado, e ela some qualquer intenção de desenho.
  //
  // Então o chão passa a ser o grafite de veio verde da marca (o mesmo do site),
  // e o lima aparece onde ele deve aparecer: no wordmark e no botão. Quem tem
  // tema próprio continua mandando na tela dele — isto é só o padrão de fábrica.
  // VAZIO É "TIRA DA MARCA", e é o padrão desde o primeiro commit — diferente
  // de uma cor igual à da marca: quem deixou vazio continua acompanhando quando
  // a marca mudar; quem escolheu a mesma cor, não.
  //
  // Em 25/08/2026 eu cravei um degradê aqui (a receita do Will, depois o Aço com
  // meio verde) e o Marlon pediu o padrão original de volta junto com a marca
  // GoFitNow. Cravar aqui foi o que fez o fundo de todo mundo virar cinza-verde.
  // ── 10/09/2026: CRAVADO em marinho, e desta vez com motivo ─────────────
  //
  // A regra que o parágrafo acima descreve é mais forte do que parecia:
  // derivar o fundo da marca só dá certo quando a MARCA É AZUL. Com lima deu
  // verde-oliva; com o laranja da VAFIT deu marrom — a tela de entrar inteira
  // virou uma mancha cor de barro, com o botão laranja sumindo dentro dela.
  //
  // O logo da VAFIT tem DOIS papéis para duas cores: o marinho é o corpo e o
  // laranja é o acento. Derivar um do outro apaga exatamente essa distinção.
  // Então o chão é o marinho do logo, cravado, e o laranja fica onde ele deve
  // ficar: no botão, no link, no foco.
  //
  // Quem tem tema próprio continua mandando na tela dele — isto é padrão de
  // fábrica, não imposição.
  gradientFrom: "#16295c",
  gradientVia: "#1a4085",
  gradientTo: "#0d1a3d",
  gradientMotion: "none",
  ...
  logo: "",
  // ── VOLTOU a 44, e o caminho até aqui é a lição ──────────────────────
  //
  // Subi para 64 e depois 96 tentando consertar um logo que parecia pequeno.
  // Estava consertando o sintoma: o arquivo é que estava errado.
  //
  // O `logo-wordmark.png` do GoFitNow era 100% TEXTO, de borda a borda — o
  // corredor morava em `logo-symbol.png`, separado. Eu troquei por uma
  // ASSINATURA (corredor + nome), e aí na mesma altura só 60% virava letra.
  // Nenhum valor de `logoSize` conserta isso sem inchar a peça inteira.
  //
  // A saída não foi tirar o corredor: foi RECOMPOR a peça com o nome na
  // altura dele. No arquivo original o nome tem 60% da altura do corredor;
  // no `logo.png` os dois têm a mesma. A peça volta a 5,71:1 — o wordmark
  // antigo era 5,57:1 — e por isso 44 devolve a mesma largura E a mesma
  // altura de letra de antes.
  //
  // Mas 44 ficou GRANDE, e a régua da dimensão explica por que a régua do
  // olho discorda: a peça antiga era letra prateada e fina, esta tem o
  // corredor em azul e laranja no meio. Mesma caixa, muito mais peso.
  // 36 é onde o conjunto pesa como o antigo pesava — medido em três
  // capturas da tela real, não estimado.
  logoSize: 36,
  ...
  // ── O FUNDO DO CARD DA LOGO ──────────────────────────────────────────────
  //
  // Um cartão atrás da logo, com degradê próprio. Existe por um problema que a
  // marca nova criou: o nosso wordmark tem "SHAPE" BRANCO, e o cliente pode
  // escolher fundo claro — aí metade do nome desaparece. Com o cartão, a logo
  // leva o próprio chão para onde for.
  //
  // Serve igual para a logo DO CLIENTE: quem tem marca clara ganha o mesmo
  // abrigo, sem precisar refazer a arte com fundo chapado.
  //
  // Vazio nos dois = sem cartão, a logo solta no fundo, como sempre foi.
  logoCardFrom: "#0c1207",
  logoCardTo: "#000000",
  ...
  menuLayout: "side",
  // A atual, porque ele gosta dela — ver HOMES_DO_ALUNO.
  homeAluno: "classica",
  menuBg: "",

  // ── O FUNDO DA ÁREA DA LOGO, no menu principal ───────────────────────────
  //
  // A faixa de cima do menu lateral, onde a logo mora. Ganha fundo próprio para
  // a marca poder ter um chão diferente do resto do menu — é o que faz uma logo
  // clara aparecer num menu claro, e o que dá à faixa um ar de cabeçalho em vez
  // de "o mesmo azul do menu com uma imagem em cima".
  //
  // Duas cores = degradê. Uma só = chapado. Vazio nos dois = sem fundo, a faixa
  // fica da cor do menu, como sempre foi.
  menuLogoBgFrom: "",
  menuLogoBgTo: "",

  // A cor dos ÍCONES da faixa (o botão de recolher, e o de fechar no celular).
  //
  // Vazio = calculada: claro sobre faixa escura, escuro sobre faixa clara. Este
  // campo existe porque o cálculo acerta o contraste e não acerta o GOSTO — e
  // porque foi o primeiro defeito que a faixa criou: com fundo preto na faixa e
  // menu claro, o botão de recolher usava o realce do MENU e ficava preto sobre
  // preto.
  menuLogoFg: "",
  menuText: "",
  menuLogo: "",
...
// ── COMO A CASA DA PESSOA ATENDIDA SE PARECE ──────────────────────────────
//
// "Conseguimos pôr lá na parte de aparência da instância, para poder escolher
// como seria a página inicial? Eu gosto da atual, mas queria poder fazer algo
// bem bonito, aí cada card seria uma daquelas opções do menu que tem lá
// embaixo."
//
// Duas escolhas, e a atual continua sendo o PADRÃO — ele gosta dela, e um
// layout novo que se instala sozinho em todo cliente é uma tela trocada sem
// ninguém pedir.
//
//   classica  o que existe hoje: o próximo compromisso em cima e quatro portas
//             com os números do que há atrás de cada uma
//   cartoes   um cartão grande por destino do menu de baixo, com o número
//             dentro e a imagem atrás
//
// É do TENANT e não da conta de quem olha: é a cara do negócio, e o aluno da
// Bruna e o da Débora não escolhem o layout um do outro.
const HOMES_DO_ALUNO = ["classica", "cartoes"];
...
// MODELOS de fundo para a faixa da logo. São pares, e existem porque escolher
// duas cores que combinam é justamente o trabalho que ninguém quer ter — a
// pergunta que a pessoa faz é "qual fica bom", não "qual hexadecimal".
//
// Todos escuros de propósito: a faixa da logo fica em cima do menu, que é escuro
// na esmagadora maioria dos temas, e um par claro ali cortaria a barra em duas.
// O primeiro é o da nossa marca.
const MENU_LOGO_BGS = [
  { key: "brand", from: "#0c1207", to: "#000000" },
  { key: "ink", from: "#18181b", to: "#09090b" },
  { key: "slate", from: "#1e293b", to: "#0f172a" },
  { key: "ocean", from: "#0c2a4d", to: "#071a33" },
  { key: "wine", from: "#3b0d1c", to: "#1a0509" },
  { key: "forest", from: "#0d2818", to: "#04120a" },
  { key: "violet", from: "#241442", to: "#0e0720" },
  { key: "copper", from: "#3a1f0b", to: "#170b03" },
];
...
// ── LOOKS COMPLETOS ────────────────────────────────────────────────────────
//
// Diferente de `PRESETS`, que mexe SÓ em cor de propósito. Um look mexe em
// fundo, véu, menu, faixa da logo e botão de uma vez — e por isso ele desfaz o
// que a pessoa montou. Não é defeito, é o serviço que ele presta: a tela pede
// confirmação antes de aplicar, e é a confirmação que separa os dois conceitos.
//
// Existem porque a pergunta real de quem abre Aparência não é "que matiz?" — é
// "como faço isso ficar bom?". Quinze campos não respondem essa pergunta; cinco
// telas prontas respondem.
const LOOKS = [
  // A MARCA, e é o primeiro de propósito: quem mexeu demais em Aparência
  // precisa de um caminho de UM clique de volta ao que sai de fábrica.
  //
  // Os valores são os mesmos de `PADRAO` — não é um look "parecido com" o
  // padrão, é o padrão. Se um dia PADRAO mudar, este tem de mudar junto, ou
  // a tela passa a oferecer "voltar ao padrão" e entregar outra coisa.
  {
    key: "vafit",
    tema: {
      brand: "#fd6121",
      ...
      menuBg: "#0f1f4a",
      menuText: "",
      // A faixa NÃO pode ficar vazia aqui, e o teste cobra: o "VA" da nossa
      // logo é branco, então faixa clara apagaria metade do nome.
      menuLogoBgFrom: "#0b1735",
      menuLogoBgTo: "#16295c",
      menuLogoFg: "",
    },
  },
...
function sanitize(entrada) {
  ...
  return {
    ...
    logoCardFrom: parseHex(e.logoCardFrom) || "",
    logoCardTo: parseHex(e.logoCardTo) || "",
    menuLayout: umDe(e.menuLayout, MENU_LAYOUTS, PADRAO.menuLayout),
    homeAluno: umDe(e.homeAluno, HOMES_DO_ALUNO, PADRAO.homeAluno),
    menuBg: parseHex(e.menuBg) || "",
    menuLogoBgFrom: parseHex(e.menuLogoBgFrom) || "",
    menuLogoBgTo: parseHex(e.menuLogoBgTo) || "",
    menuLogoFg: parseHex(e.menuLogoFg) || "",
