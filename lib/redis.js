...
// ── APAGAR POR PADRÃO ─────────────────────────────────────────────────────
//
// Usado quando uma escrita alcança MUITAS linhas de uma vez e não dá para saber
// quais — aí o certo é derrubar o cache inteiro daquele cliente em vez de
// adivinhar. Acontece pouco: escrita em `users` quase sempre mira um `_id`.
//
// `scanIterator` e não `KEYS`: o segundo trava o Redis inteiro enquanto varre, e
// isso é o tipo de coisa que só dói no dia em que a base cresce.
async function esquecerPorPadrao(padrao) {
  const c = await pegar();
  if (!c) return null;
  ...
  try {
    ...
    // ── O ITERADOR ENTREGA LOTES, NÃO CHAVES ────────────────────────────
    //
    // Nesta versão do node-redis, cada volta do `scanIterator` devolve um ARRAY
    // com as chaves daquele lote. Tratá-lo como uma chave só passa um array para
    // o `del`, que responde:
    //
    //   "arguments[1]" must be of type "string | Buffer", got object instead
    //
    // E o erro é engolido pelo `catch` — a limpeza simplesmente não acontece, em
    // silêncio. Foi assim que o caso do `updateMany` sem `_id` falhou na prova em
    // produção enquanto os outros dois passavam.
    //
    // Aceita as duas formas porque versões diferentes do cliente fazem
    // diferente, e esta função não vale uma exigência de versão.
    for await (const lote of c.scanIterator({ MATCH: PREFIXO + padrao, COUNT: 200 })) {
      for (const chave of Array.isArray(lote) ? lote : [lote]) achadas.push(chave);

      // Teto de segurança: se um padrão largo demais casar com meio Redis, é
      // melhor apagar um pedaço e seguir do que segurar o processo aqui.
      if (achadas.length >= 5000) break;
    }

    if (achadas.length) await c.del(achadas);
...
// ── MARCAR SÓ SE ESTIVER LIVRE — e é isso que elege um dono ──────────────
//
// `SET NX PX`: grava se a chave NÃO existir, numa operação atômica do Redis.
// Devolve `true` para quem chegou primeiro e `false` para todos os outros.
//
// É diferente de `marcarComPrazo` logo acima, e a diferença é o ponto. Aquele
// sempre grava — serve para "não repita antes de X", onde dois gravando o mesmo
// valor não muda nada. Este serve para "só UM faz", onde ler-e-depois-gravar
// perderia a corrida: dois processos leem "está livre" no mesmo milissegundo e
// os dois seguem.
//
// Quem usa: a rotina diária, para uma máquina só publicar o tique do dia. Sem
// Redis (`null`), quem chama decide — e a rotina decide seguir, porque duas
// publicações do mesmo dia são absorvidas pelo índice único da cobrança.
async function marcarSeLivre(chave, janelaMs) {
  const c = await pegar();
  if (!c) return null;

  try {
    const r = await c.set(PREFIXO + chave, String(Date.now()), { PX: janelaMs, NX: true });
    return r === "OK";
  } catch (erro) {
    reclamar(erro);
    return null;
