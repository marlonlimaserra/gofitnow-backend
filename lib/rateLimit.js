...
// ── COM REDIS, O CONTADOR É DE TODAS AS MÁQUINAS ─────────────────────────
//
// O caminho do cluster acima resolve entre PROCESSOS da mesma máquina. Entre
// máquinas ele não alcança: cada uma teria o seu primário, e o limite de 60
// valeria 60 vezes o número de servidores.
//
// Quando `REDIS_URL` existe, a janela deslizante mora lá. É a mesma conta —
// apagar o que saiu da janela, contar o que sobrou — feita num lugar que todos
// enxergam.
//
// Redis mudo CAI DE VOLTA no caminho local. Não é o ideal (o limite volta a ser
// por máquina), mas é infinitamente melhor que recusar tudo: o limite existe
// para conter abuso, não para ser o motivo de a API parar.
async function checkNoRedis(chave, limite) {
  const r = await redis.registrarNaJanela(`rate:${chave}`, JANELA_MS);
  if (!r) return null;

  if (r.quantas > limite) {
    const primeira = await redis.maisAntigaNaJanela(`rate:${chave}`, JANELA_MS);
    const liberaEm = primeira ? primeira + JANELA_MS - r.agora : JANELA_MS;
  ...
  return { allowed: true, limit: limite, remaining: limite - r.quantas, retryAfter: 0 };
}

async function checkShared(chave, limite = LIMITE_PADRAO) {
  if (redis.ligado()) {
    const r = await checkNoRedis(chave, limite);
    if (r) return r;
  }
