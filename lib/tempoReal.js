...
// veria nada.
//
// ── O DIA CHEGOU (30/08/2026) ─────────────────────────────────────────────
//
// Este comentário dizia "no dia do segundo núcleo, isto precisa de um
// adaptador". O Redis entrou, e com ele o adaptador: `createAdapter` faz cada
// `emit` virar uma publicação que TODAS as máquinas recebem e reenviam para as
// conexões que elas seguram.
//
// Sem `REDIS_URL` nada muda — o socket.io continua emitindo em memória, que é o
// certo quando existe um processo só.
let io = null;
...
function iniciar(servidorHttp, app) {
  ...
  // ── O ADAPTADOR ENTRA DEPOIS, e sem segurar o boot ──────────────────────
  //
  // Ele é ligado de forma assíncrona e o servidor não espera por ele. A razão é
  // a mesma que vale para o resto do Redis aqui: um serviço auxiliar fora do ar
  // não pode impedir a API de subir. Sem o adaptador, o tempo real volta a
  // alcançar só quem está conectado NESTA máquina — degradado, não parado.
  //
  // O par é dedicado: o cliente que assina um canal entra em modo de assinatura
  // e para de atender comando comum. Compartilhar o cliente dos contadores faria
  // eles pararem de responder no instante em que o tempo real subisse.
  if (redis.ligado()) {
    redis
      .parAdaptador()
      .then((par) => {
        if (!par || !io) return;
        io.adapter(createAdapter(par.pub, par.sub));
        console.log("[tempoReal] adaptador do Redis ligado — o aviso alcança todas as máquinas");
      })
      .catch((erro) => console.error("[tempoReal] sem adaptador:", erro?.message));
  }
