...
// duas precisam escrever o mesmo número, senão a folha discorda do que estava
// na tela de quem a imprimiu.
//
// Morava dentro de `documentoFinanceiro`, e subiu para cá quando as listas
// exportáveis (equipamentos, funcionários, cobranças) passaram a precisar da
// mesma regra. Duas cópias seriam duas maneiras de escrever o mesmo valor.
function dinheiro(centavos, moeda) {
  ...
  try {
    ...
    // Moeda que o `Intl` desta máquina não conhece não pode derrubar a folha:
...
// ── QUANDO A FOLHA FOI EMITIDA: data E HORA ─────────────────────────────
//
// *"coloque horário"*. A data sozinha não distingue duas emissões do mesmo dia,
// e é justamente no mesmo dia que elas acontecem — alguém manda o extrato, o
// cliente paga, e o segundo extrato sai meia hora depois. Com só a data, os
// dois papéis são indistinguíveis e ninguém sabe qual é o atual.
//
// Sem o atalho do `soDia` de propósito: um instante de emissão é um instante,
// nunca um dia solto, e formatá-lo em UTC mostraria a hora do servidor.
function formatarDataHora(valor, lang, fuso) {
  if (!valor) return "";
  const d = new Date(valor);
  if (Number.isNaN(d.getTime())) return "";
...
function secao(titulo, conteudo) {
  ...
  // TÍTULO VAZIO É UMA SEÇÃO SEM CABEÇALHO, de propósito.
  //
  // Nem toda seção precisa se anunciar: a tabela principal de uma folha já é
  // explicada pelo cabeçalho das colunas, e um título em cima dela só rouba a
  // linha que a separa do que veio antes. Um `h2` vazio deixaria o espaço dele.
...
module.exports = {
  TINTA, FRACO, LINHA, escapar, numero, dinheiro, formatarData, formatarDataHora, secao, cartoes, pagina,
};
