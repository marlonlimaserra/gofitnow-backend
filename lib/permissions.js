...
const GROUPS = [
  ...
  {
    // A SUPLEMENTAÇÃO é separada da dieta de propósito. Numa clínica com
    // nutricionista e treinador, quem prescreve whey e creatina não é
    // necessariamente quem monta o plano alimentar — e o contrário também
    // acontece. Uma chave só obrigaria a dar as duas para conceder uma.
    key: "supplements",
    items: [
      { key: "supplements.view" },
      { key: "supplements.manage" },
    ],
  },
  {
    // A ANAMNESE é a conversa da primeira consulta, guardada: doenças,
    // medicamentos, alergias, hábitos. É o dado mais sensível da ficha depois da
    // prescrição — e é por isso que ver e preencher são chaves separadas.
    key: "anamnesis",
    items: [
      { key: "anamnesis.view" },
      { key: "anamnesis.manage" },
    ],
  },
  {
    // Os EXAMES são a peça que fecha o ciclo do acompanhamento (anamnese →
    // conduta → exame → ajuste). Separados da avaliação física porque são
    // trabalhos diferentes: a avaliação o profissional mede; o exame vem de um
    // laboratório, e quem lança um laudo não é necessariamente quem faz dobras.
    key: "exams",
    items: [
      { key: "exams.view" },
      { key: "exams.manage" },
  ...
  {
    // A PRESCRIÇÃO é a mais restrita das três: é documento assinado, com
    // registro de conselho. Ver o que foi prescrito é uma coisa (a recepção
    // pode precisar reimprimir); emitir é outra, e só quem assina.
    key: "prescriptions",
    items: [
  ...
  },
  {
    // ── A EQUIPE DA CASA ──────────────────────────────────────────────────
    //
    // Três chaves, e a terceira é o motivo de este módulo ter permissão própria
    // em vez de pegar carona no financeiro.
    //
    // `view` abre a lista e a ficha; `manage` cadastra, lança advertência e
    // preenche o ponto. `payroll` é o SALÁRIO — e ele é separado porque a
    // recepcionista que preenche o ponto de todo mundo não pode ver quanto o
    // gerente ganha.
    //
    // Sem `payroll` o servidor NÃO MANDA o número. Esconder na tela seria
    // deixá-lo viajar no JSON, a um F12 de distância.
    key: "employees",
    items: [
      { key: "employees.view" },
  ...
  },
  {
    // ── A ESTRUTURA ───────────────────────────────────────────────────────
    //
    // Equipamentos, manutenção e estoque. Chave própria, e não a do financeiro:
    // o gesto de todo dia aqui é "saiu um galão de desinfetante", e quem o faz
    // é a faxineira ou a recepção — que não têm, nem devem ter, acesso ao
    // dinheiro da casa.
    //
    // Exigir a chave do financeiro para dar baixa faria a baixa não acontecer,
    // e um estoque que ninguém baixa é um número errado com cara de certo.
    //
    // DAR BAIXA exige só `view`: é o uso, não a administração. `manage` é para
    // cadastrar equipamento e insumo — ver `controllers/Structure.js`.
    key: "structure",
    items: [
      { key: "structure.view" },
