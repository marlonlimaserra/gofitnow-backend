...
async function abrir() {
  ...
  abrindo = p
    .launch({
      ...
      //
      // Não há `--no-sandbox` NA LISTA: o serviço roda como `gofitnow`, não como
      // root, então o sandbox FUNCIONA — e ele é o que separa o decodificador de
      ...
      //
      // ── E POR QUE EXISTE UMA VÁLVULA (07/09/2026) ────────────────────────
      //
      // No Fargate não há user namespace disponível, e aí o sandbox do Chromium
      // não consegue nem inicializar: o navegador morre no `launch`, e o sintoma
      // é a rota de PDF respondendo erro sempre, em toda máquina nova.
      //
      // `PUPPETEER_ARGS` acrescenta o que aquele ambiente precisa — e a decisão
      // de abrir mão do sandbox fica no ambiente que abriu mão, declarada na
      // definição da task, e não escondida aqui num `--no-sandbox` que valeria
      // para o VPS também.
      args: [
        "--disable-dev-shm-usage",
        "--disable-gpu",
        "--font-render-hinting=none",
        ...String(process.env.PUPPETEER_ARGS || "")
          .split(/[\s,]+/)
          .filter(Boolean),
      ],
