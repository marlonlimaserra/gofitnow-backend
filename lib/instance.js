...
function fromRequest(req, baseDomain = "gofitnow.fit") {
  ...
  if (host.endsWith(sufixo)) {
    ...
    if (!rotulo.includes(".")) {
      ...
      // `app.gofitnow.fit` é o produto, não um cliente. Ele cai em RESERVADOS
      // por outra porta: `app` não está na lista, então é barrado aqui.
      if (nome && nome !== "app" && nome !== "www" && nome !== "api") return nome;
    }
  }
