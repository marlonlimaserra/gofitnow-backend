...
// `baseDomain` virou LISTA: `bruna.shapeapp.fit` e `bruna.gofitnow.fit` levam à
// mesma cliente desde 25/08/2026. Aceita string por compatibilidade — os testes
// e quem chamava com um domínio só continuam valendo.
function fromRequest(req, baseDomain = require("./domain.js").BASE_DOMAINS) {
  ...
  const bases = Array.isArray(baseDomain) ? baseDomain : [baseDomain];
  const base = bases.find((d) => host.endsWith("." + d));
  if (base) {
    const rotulo = host.slice(0, -("." + base).length);
