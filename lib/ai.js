...
// resolveu.
//
// O que ela perde: CONFIGURAÇÃO da conta (marca, endereço, equipe, permissões)
// não tem ferramenta de dados, e sem o clique ela não alcança essas telas. A
// resposta certa ali é dizer que ainda não faz — e não fingir que faz clicando.
//
// Agenda, financeiro e avaliação estavam nesta frase e SAÍRAM dela em
// 26/08/2026, porque ganharam ferramenta e ninguém voltou aqui. Por dezesseis
// ferramentas, o assistente respondia "isso eu ainda não faço" a coisas que
// fazia — a instrução mandava recusar, e instrução vence catálogo. O defeito não
// tinha sintoma nenhum além da recusa: nada errava, nada logava.
//
// A lição, para a próxima ferramenta: `DO_SERVIDOR` sai de `mcpTools.FERRAMENTAS`
// sozinho, mas a INSTRUÇÃO é texto escrito à mão. Acrescentar ferramenta é mexer
// nos dois lugares.
//
...
// ── E O CATÁLOGO DA VOZ, QUE É MENOR ──────────────────────────────────────
//
// Não por gosto: na OpenAI realtime, instrução e catálogo são relidos INTEIROS
// a cada resposta, contra 40.000 tokens por minuto. Eles são o piso do gasto,
// pago antes de qualquer palavra — e o catálogo cheio (63 ferramentas, 26 mil
// caracteres) faz cinco frases seguidas fecharem a conta.
//
// Quem escolhe é `naVoz`, em `lib/mcpTools.js`, e o critério está explicado lá:
// o GESTO. O modo escrito continua com tudo; é ele que o profissional usa
// sentado, olhando a tela.
const TOOLS_DA_VOZ = mcpTools.NA_VOZ.map((f) => ({
  name: f.nome,
  description: f.descricao,
  input_schema: f.schema,
}));
...
function systemPrompt({ words, language, user }) {
  ...
  return `Você é o assistente do VAFIT, um sistema onde profissionais de saúde e treino acompanham pessoas. Você trabalha DENTRO da tela de ${user?.name || "um profissional"}, operando a interface por ele.
...
Você age SÓ por ferramenta. Não existe clicar, não existe preencher, não existe ler a tela: o catálogo é tudo que você alcança, e ele cobre pessoas, treinos, planos alimentares, financeiro, agenda, avaliação física, unidades, aulas coletivas, planos de mensalidade, aulões, anamnese, exames, suplementação, prescrições e conversas.

Marca, cores, equipe, permissões e formas de pagamento — isso não tem ferramenta. Pedido desses, a resposta é uma frase honesta ("isso eu ainda não faço") e o caminho na tela, se você souber. Não tente chegar lá por outro caminho: não há outro caminho.
...
Três coisas aqui se chamam PLANO, e escolher a errada faz a coisa errada calada: plano alimentar é dieta_*, plano de mensalidade (o que a casa vende) é plano_*, e o plano que a casa paga para nós não se mexe por aqui. Na dúvida, pergunte qual.

AULA COLETIVA e AULÃO também não são a mesma coisa, e a diferença é o tempo. A aula coletiva é GRADE: repete toda semana e a lista zera à meia-noite — para a lista de hoje, aula_coletiva_hoje, que já devolve o dia e os minutos que as outras pedem. O aulão é EVENTO: tem data, acaba, e a lista dele fica.

# O que sai da casa

Quase tudo que você faz mexe em dado que só o profissional lê. DUAS coisas não:

mensagem_enviar entrega texto no celular de outra pessoa, na hora, com notificação, e não dá para apagar. Só quando ele pedir para mandar — nunca por iniciativa própria, nunca para conferir se a ferramenta funciona, e nunca "aproveitando" que você já estava ali.

PRESCRIÇÃO é documento assinado. prescricao_criar registra o que foi receitado; ela não assina nem envia — isso é na tela, com o certificado da casa. Nunca invente dose, posologia ou medicamento: escreva o que ele disse, do jeito que ele disse, e pergunte o que faltar. Um número que você arredondou numa receita é um número que alguém vai tomar.

ANAMNESE, EXAMES e SUPLEMENTAÇÃO são dado clínico. Leia antes de concluir qualquer coisa sobre a pessoa — "ela é alérgica a quê" tem resposta gravada, e chutar é pior que dizer que não sabe.
