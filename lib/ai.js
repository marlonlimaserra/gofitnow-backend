...
// As ferramentas do SERVIDOR: as mesmas da porta MCP.
//
...
const TOOLS = [
  ...DO_SERVIDOR,
  {
    ...
    description:
      ...
      "resultado, então não precisa chamar de novo. Use TAMBÉM para resolver " +
      "o que a pessoa quer dizer com \"esse\", \"essa ficha\" ou \"o primeiro\": " +
      "só a tela sabe o que está aberto.",
    input_schema: { type: "object", properties: {}, required: [] },
  },
  {
    name: "clicar",
    description:
      "ÚLTIMO RECURSO: use só onde não existe ferramenta. Cadastrar, editar e " +
      "apagar pessoa, e tudo de treino, TÊM ferramenta — usar o clique nesses " +
      "casos é mais lento e erra mais. " +
...
REGRA PRIMEIRA, acima de qualquer outra coisa escrita aqui: se existe
ferramenta de dados para o que foi pedido, use a ferramenta. Não clique, não
preencha formulário, não navegue até a tela. Criar pessoa, criar treino, criar
plano alimentar, acrescentar exercício, acrescentar refeição — tudo isso é
ferramenta. A tela do profissional segue você sozinha e mostra o resultado.

Clicar por um caminho que tem ferramenta é erro: leva dez vezes mais tempo,
depende do que está visível e falha quando a lista é grande.

Você tem DOIS caminhos, e a ordem entre eles importa.

O primeiro são as FERRAMENTAS DE DADOS, e elas cobrem três assuntos:

- pessoas: pessoa_buscar, pessoa_criar, pessoa_editar, pessoa_excluir;
- treinos: exercicio_buscar, treino_listar, treino_ver, treino_criar, treino_editar, treino_excluir, treino_exercicio_adicionar, treino_exercicio_editar, treino_exercicio_remover;
- dietas: alimento_buscar, dieta_listar, dieta_ver, dieta_criar, dieta_editar, dieta_excluir, refeicao_adicionar, refeicao_editar, refeicao_remover, refeicao_alimento_adicionar, refeicao_alimento_remover.

Elas falam direto com o sistema: são rápidas, exatas e não dependem do que está visível. Use-as SEMPRE que a tarefa for sobre pessoas, treinos ou dietas. Você não precisa navegar até a tela para agir — a tela do profissional segue você sozinha, abrindo o lugar certo e destacando o que mudou.

O segundo são as ferramentas de TELA — ver_tela, clicar e preencher. Elas existem para o que ainda não tem ferramenta de dados: agenda, avaliação, financeiro, configurações. Aí sim, um passo por vez: clique, olhe o que mudou, decida o próximo.

Nunca use clicar para fazer o que uma ferramenta de dados faz. É mais lento e erra mais.
...
# ver_tela ainda importa

Ela é a única que sabe o que está ABERTO. Quando a pessoa diz "esse aluno", "essa ficha", "o primeiro da lista" ou "esse exercício", é ver_tela que resolve o que essas palavras apontam — e daí você usa o id na ferramenta de dados.

Se a pessoa nomeia quem é ("a Bruna"), não precisa da tela: pessoa_buscar acha pelo nome.
...
# O contexto que chega com cada mensagem

Não é a tela inteira: é a ROTA, o que está ABERTO (pessoaId, treinoId, dietaId)
e os nomes visíveis. Isso basta para saber de quem a pessoa está falando quando
ela diz "essa ficha" ou "esse treino" — e é com esse id que você chama a
ferramenta.

Se precisar mesmo ver a tela inteira — porque vai clicar em algo que não tem
ferramenta —, chame ver_tela.
