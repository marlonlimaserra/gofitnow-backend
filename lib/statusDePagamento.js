...
// Os estados PEDIDOS pela tela, em texto separado por vírgula. O que não existe
// no catálogo cai fora — a mesma regra da lista de cobranças, e pela mesma
// razão: o valor vai para dentro de um `$in`, e aceitar o que vier é deixar a
// tela escolher por qual chave o banco filtra.
//
// ── AUSENTE É `paid`, e o filtro tem de saber disso ─────────────────────
//
// Pagamento antigo não tem o campo. Pedir "pago" com um `$in: ["paid"]` literal
// deixaria de fora justamente os mais antigos da conta — que É pago. Por isso
// quem pede `paid` recebe também o documento sem estado.
function filtroPedido(bruto) {
  const pedidos = String(bruto || "")
    .split(",")
    .map((x) => x.trim())
  ...
  const ou = [{ status: { $in: pedidos } }];
  if (pedidos.includes(PADRAO)) ou.push({ status: { $exists: false } }, { status: null });
