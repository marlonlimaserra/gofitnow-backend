...
async function conectar() {
  ...
  ligando = (async () => {
    try {
      ...
      conexao.on("close", () => {
        // Esquece o que morreu; a próxima chamada reabre. Não há laço de
        // reconexão aqui de propósito: quem publica é um tique diário e quem
        // consome tem o próprio religar — um laço a mais só disputaria com eles.
        conexao = null;
        canal = null;
      });
...
async function consumir(aoReceber) {
  ...
  if (!c) {
    ...
    setTimeout(() => consumir(aoReceber), ESPERA_PARA_RELIGAR_MS).unref?.();
    return false;
  }
