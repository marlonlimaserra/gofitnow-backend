...
// AS DUAS CORES, e não uma.
//
// E-mail não tem variável de CSS nem JavaScript: o par tem de ser resolvido
// aqui. E o lima da marca é uma cor CLARA — usá-la como antes usávamos o azul
// daria wordmark ilegível no branco (1,5:1) e botão lima com texto branco
// (1,5:1). Então: lima é FUNDO de botão, grafite é TEXTO.
const LIMA = "#b5e01f";
const GRAFITE = "#16200c";
...
<body style="margin:0;padding:24px;background:#f1f5f9;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,Arial,sans-serif;">
  <div style="max-width:520px;margin:0 auto;background:#ffffff;border-radius:12px;padding:32px;border:1px solid #e2e8f0;">
    <div style="font-size:22px;font-weight:800;color:${GRAFITE};margin-bottom:24px;">GoFitNow</div>
    ...
    ${
      buttonUrl
        ? `<div style="margin:28px 0;">
             <a href="${buttonUrl}" style="display:inline-block;background:${LIMA};color:${GRAFITE};text-decoration:none;font-size:14px;font-weight:600;padding:12px 24px;border-radius:8px;">${buttonLabel}</a>
           </div>
...
function anamnesisInvite({ lang, name, professional, url, days }) {
  return build("anamnesis", lang, { name, professional, url, days });
}
