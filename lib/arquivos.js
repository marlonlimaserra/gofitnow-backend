...
const PASTAS = {
  ...
  tickets: "tickets",
  // A capa e a galeria de um aulão. Collection PRÓPRIA (`aulao_images`) e pasta
  // própria: a coleta de lixo da marca apaga o que o tema não referencia, e
  // apagaria a foto de todo aulão no primeiro salvamento de aparência.
  //
  // ESTA LINHA FALTAVA (16/09/2026), e a falta derrubou o upload com
  // "arquivos: pasta desconhecida". A tabela recusar o que não conhece é o
  // comportamento certo — inventar o caminho deixaria um cliente escrevendo
  // numa pasta que ninguém declarou. O erro foi meu, em não registrar.
  aulaoes: "aulaoes",
  // A CAPA de um plano — a foto no topo do cartão da vitrine. Pasta própria
  // pela mesma razão do aulão: a coleta de lixo da marca apaga o que o tema não
  // referencia, e a capa de um plano não está no tema.
  planos: "planos",
  // ── AS QUATRO QUE FALTAVAM (19/09/2026) ────────────────────────────────
  //
  // Elas nasceram com `lib/modeloDeImagens.js` — a fábrica que unificou os
  // modelos de foto — e NENHUMA foi registrada aqui. O resultado: unidade,
  // aula coletiva, fornecedor e funcionário aceitavam a foto na tela e
  // estouravam "pasta desconhecida" no servidor, desde o dia em que cada um
  // subiu. Três deles com zero fotos gravadas e ninguém notou, porque ninguém
  // tinha tentado subir uma.
  //
  // É a MESMA falta do aulão, quatro linhas acima, três dias depois. A terceira
  // vez pede um teste, e ele existe: `test/lib/pastasDeArquivo.test.js` recusa
  // um `prefixo` que não esteja nesta tabela.
  unidades: "unidades",
  aulas: "aulas",
