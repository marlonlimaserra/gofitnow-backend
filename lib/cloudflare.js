...
const { BASE_DOMAINS } = require("./domain.js");
...
// O curinga vale para OS DOMÍNIOS NOSSOS, e só. Host que não termina em nenhum
// deles é domínio próprio de cliente: aí não há curinga cobrindo, e os passos de
// sempre continuam valendo.
//
// ── Por que a lista, e não o BASE_DOMAIN (13/09/2026) ─────────────────────
//
// Aqui era `endsWith("." + BASE_DOMAIN)`, e isso valeu enquanto o canônico não
// mudou. Quando ele virou `vafit.app`, `novo.gofitnow.fit` deixou de contar como
// coberto pelo curinga — e o efeito não seria um erro, seria PIOR: o código
// voltaria a tentar criar registro de DNS e domínio personalizado no Pages para
// um host que a rota do Worker já atende, gastando chamada, exigindo credencial
// e podendo recriar a precedência de Pages sobre Worker que já custou três
// deploys sem efeito em agosto.
//
// A rota curinga existe nas TRÊS zonas (ver `gofitnow-frontend/wrangler.toml`),
// então a pergunta certa é "é um domínio nosso?", não "é o canônico de hoje?".
function usaCuringa(host, env) {
  const c = config(env);
  if (!c.curinga) return false;
  const limpo = String(host || "").toLowerCase();
  return BASE_DOMAINS.some((base) => limpo.endsWith("." + String(base).toLowerCase()));
}
